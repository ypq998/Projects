...
var getOpt = function(){
    var opt = {
        items_per_page: 20,	//每页记录数
        num_display_entries: 3, //中间显示的页数个数 默认为10
        current_page:0,	//当前页
        num_edge_entries:2, //头尾显示的页数个数 默认为0
...
function fillData(data){	 
    ...
    $.each(data,function(k,v){    	
        var html = "" ;
        html += '<tr> ' +
                '<td>'+ (v.createTime==null?'': getSmpFormatDateByLong(v.createTime,false)) +'</td>' +
                '<td>'+ (v.item ==null || v.item.name==null?'':v.item.name) +'</td>' +
                '<td>'+ (v.qty==null?'':v.qty) +'</td>' +
...
function create(){   
    $.get("/actor/new",{ts:new Date().getTime()},function(data){
        art.dialog({
            lock:true,
            opacity:0.3,
            title: "新增",
